...
class Program
{
    static void Main()
    {
        ...
        while (!gameOver)
        {
            ...
            if (input.Length == 2 && int.TryParse(input[0], out int x) && int.TryParse(input[1], out int y))
            {
                ...
                {
                    Console.WriteLine("Nieprawidłowe współrzędne lub pole już odkryte.");
                    Console.ReadKey();
                }
            }
            else
            {
                Console.WriteLine("Nieprawidłowe dane wejściowe. Naciśnij dowolny klawisz...");
                Console.ReadKey();
            }
        }
    }
    ...
    static bool CheckWin(char[,] displayBoard, int bombCount)
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                if (displayBoard[i, j] != ' ') uncoveredCount++;
            }
        }
